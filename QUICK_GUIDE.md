...
Install the checkout and Chromium, then run `mcp-vision demo` on the intended executor. The demo needs no model or credentials. Run `mcp-vision install` to register Claude Desktop, Cursor, and an installed Codex client, or configure another MCP host to launch `mcp-vision serve` using an absolute executable path. Add `--allow-browser-writes` only when the operator wants routine input; recognized sensitive actions still need confirmation. On a headless executor, those actions block without a confirmer.
