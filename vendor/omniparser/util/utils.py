...
paddle_ocr = PaddleOCR(lang='en')  # v3 API: removed show_log, use_angle_cls, etc.
import time
import base64
...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    ...
    if model_name == "blip2":
        ...
        # MPS doesn't support float16 ops the same way CUDA does - use float32
    ...
    for i, box1_elem in enumerate(boxes):
        ...
        if is_valid_box:
            if ocr_bbox:
                ...
                filtered_boxes.append(box1_elem)  # was box1 (bare coords), needs full dict
...
def get_som_labeled_img(image_source: Union[str, Image.Image], model=None, BOX_TRESHOLD=0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None, scale_img=False, imgsz=None, batch_size=128):
    ...
    if ocr_bbox:
        ...
        print('no ocr bbox!!!')
        ocr_bbox = []  # was None, but zip(None, ...) crashes below
