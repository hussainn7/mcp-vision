...
def run_contextual_ui(*, port: int = 7331, provider: str | None = None, live_driver: str = "native", cdp_endpoint: str | None = None) -> None:
    ...
    class Controller(AppKit.NSObject):
        ...
        def _build_panel(self):
            ...
            self.input.setBezeled_(False)
            self.input.setDrawsBackground_(True)
            self.input.setBackgroundColor_(AppKit.NSColor.colorWithCalibratedRed_green_blue_alpha_(.12, .15, .20, 1))
            self.input.setFocusRingType_(AppKit.NSFocusRingTypeNone)
            self.input.setWantsLayer_(True)
            self.input.layer().setCornerRadius_(9)
            self.input.layer().setBorderWidth_(1)
            self.input.layer().setBorderColor_(AppKit.NSColor.colorWithCalibratedRed_green_blue_alpha_(.25, .30, .38, 1).CGColor())
